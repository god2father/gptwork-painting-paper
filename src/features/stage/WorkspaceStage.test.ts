...
describe('WorkspaceStage', () => {
  it('renders one overhead atelier instead of a three-column viewer', async () => {
    ...
    expect(html.match(/stage-layer__facet--left/g)).toHaveLength(6)
    expect(html.match(/stage-layer__facet--right/g)).toHaveLength(6)
    expect(html.match(/stage-layer__crease/g)).toHaveLength(6)
